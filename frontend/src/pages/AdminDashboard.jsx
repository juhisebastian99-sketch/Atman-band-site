...
const Field = ({ label, ...props }) => (
  <div>
    <label className="text-[0.65rem] tracking-[0.3em] uppercase text-[#C9A227]">
      {label}
    </label>
...
export default function AdminDashboard() {
  ...
  const [tab, setTab] = useState("settings"); // settings | videos | inquiries
  ...
  const addVideo = async () => {
    if (!newVid.youtube_id || !newVid.title) {
      return toast.error("YouTube ID and title are required.");
    }
    try {
      const { data } = await api.post("/admin/videos", {
        ...newVid,
        order: Number(newVid.order) || 0,
      });
              ...
              return (
                <button
                  ...
                  className={`inline-flex items-center gap-2 px-5 py-2.5 border text-[0.7rem] tracking-[0.28em] uppercase transition-colors ${
                    active
                      ? "bg-[#C9A227] text-[#121212] border-[#C9A227]"
        ...
        <div className="mt-10">
          {loading ? (
            ...
            <motion.div
              ...
              className="bg-[#1a1a1a] border border-[#C9A227]/15 p-8 lg:p-10"
            ...
            >
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <Field
                  ...
                  placeholder="hello@atmanmusic.co"
                />
                <Field
                  label="Phone"
                ...
                <Field
                  ...
                  placeholder="Mumbai · Available worldwide"
                />
                <Field
                  label="Instagram URL"
              ...
              <div className="mt-10 pt-6 border-t border-[#C9A227]/15">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <Field
                    ...
                    placeholder="Music That Touches The Soul"
                  />
                  <Field
                    label="Hero Subline"
                    data-testid="settings-hero-subline"
                    value={settings.hero_subline}
                    onChange={(e) => setSettings({ ...settings, hero_subline: e.target.value })}
                    placeholder="Luxury Live Music for…"
                  />
                </div>
              </div>
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4 }}
              data-testid="admin-videos-panel"
            >
              <div className="bg-[#1a1a1a] border border-[#C9A227]/15 p-8 lg:p-10 mb-8">
                ...
                <div className="grid md:grid-cols-4 gap-6">
                  <Field
                    label="YouTube ID"
                    data-testid="new-video-id"
                    value={newVid.youtube_id}
                    onChange={(e) => setNewVid({ ...newVid, youtube_id: e.target.value })}
                  ...
                  <Field
                    ...
                    placeholder="Sufi Evening · Live Set"
                  />
                  <Field
                    label="Subtitle"
                    data-testid="new-video-subtitle"
                    value={newVid.subtitle}
                    onChange={(e) => setNewVid({ ...newVid, subtitle: e.target.value })}
                    placeholder="Palace Wedding · Udaipur"
                  />
                  <Field
                    label="Order"
                    data-testid="new-video-order"
                    type="number"
                    value={newVid.order}
                    onChange={(e) => setNewVid({ ...newVid, order: e.target.value })}
                    placeholder="0"
                  />
                </div>
                <button
                  onClick={addVideo}
                  data-testid="new-video-add"
                  ...
                  >
                    ...
                    <div className="lg:col-span-2">
                      <Field
                        label="YouTube ID"
                        value={v.youtube_id}
                        onChange={(e) => updateVideo(v.id, { youtube_id: e.target.value })}
                        onBlur={(e) => updateVideo(v.id, { youtube_id: e.target.value })}
                      />
                    </div>
                    <div className="lg:col-span-3">
                      <Field
                    ...
                    <div className="lg:col-span-2">
                      <Field
                        label="Subtitle"
                        value={v.subtitle || ""}
                        onChange={(e) => updateVideo(v.id, { subtitle: e.target.value })}
                      />
                    </div>
                    <div className="lg:col-span-1">
                      <Field
                        ...
                        type="number"
                        value={v.order}
                        onChange={(e) => updateVideo(v.id, { order: e.target.value })}
                      />
                    </div>
            ...
            <motion.div
              ...
              transition={{ duration: 0.4 }}
              className="bg-[#1a1a1a] border border-[#C9A227]/15 p-8 lg:p-10"
              data-testid="admin-inquiries-panel"
            >
              <div className="flex items-center gap-3 mb-6">
                  ...
                  {inquiries.map((i) => (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="mt-1 text-[#F8F6F2]/85">
                          {i.event_date || "—"} · {i.guest_count || "—"}
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="mt-1 text-[#F8F6F2]/75 whitespace-pre-line">
                          {i.message || "—"}
                        </div>
